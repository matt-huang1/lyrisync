...
- **Compact** — an optional layout that reduces the window to the line being sung, with the romanisation under it if that layer is on. A wide thin strip, 79 points tall at the default text size against the full layout's 183. The loop and speak controls go away with the other rows and come back when the pointer is over the window. A sync pass takes the full layout back for as long as it runs, and gives the strip back afterwards.
- **The strip has its own text size** — five sizes in the menu, from 14 to 28 points. Everywhere else the text scales with the window, which in a one-line strip means a wider window shows the same words in bigger type. Naming the size instead gives the width its job back: widen the strip and you see more of the line. Its height follows the size, so there is nothing to drag vertically.
- **The strip fits the song** — in the compact layout the window sizes itself to the track when its lyrics arrive, and then holds still: as narrow as it can be without moving again for the rest of the song. Capped by the screen, anchored on its own centre, and off the moment you drag an edge. On by default in compact, which is the only place it acts.
- **Dock to top** — puts the window centred just under the menu bar on whichever screen it is on, clear of the notch, and squares off its top corners so it reads as the menu bar's own band continuing downwards rather than a panel parked below it. A command, not a snap: nothing holds it there, it is as draggable afterwards as before, and it is round again the moment you move it.
...
- **Yield to the pointer** — an optional setting, off by default, for the times the window is on top of the thing you are reaching for. **Dodge** steps it out of its own rectangle while your pointer is there and puts it back, exactly, when you leave. **Ghost** leaves it where it is, fades it until the work underneath is 86% untouched, and lets the clicks straight through to whatever is behind it. Both suspend themselves while you need to click the window itself, and both work in either layout and while docked.
...
- **Publish a sync back** — when LRCLIB has the words for a song but no timings, you can offer yours. One song at a time, never automatically, and only after you have read the exact submission on screen.
...
- **Drag** anywhere to move, **drag the edges** to resize (text scales with width in the full layout; in the strip the width decides how much of the line fits, and dragging hands the width back from the song), **scroll** to dim. In the plain-lyrics view, scroll moves the lyrics and Option+scroll dims.
...
- **Publish this sync to LRCLIB…** appears when LRCLIB is holding the plain lyrics for the song and no timings, and you have made some. It opens a window showing exactly what would be sent — the metadata, your synced lyrics and LRCLIB's own plain lyrics — and sends nothing until you press the button under it. LRCLIB asks for a few seconds of CPU rather than an account, so there is a running count and a Cancel. Once a sync has gone the entry is replaced by **Published to LRCLIB**; re-syncing the song offers it again. Your files in `.user_syncs/` are copied outward and never changed. [More](docs/publishing.md).
...
A worker thread follows the Spotify desktop app over AppleScript, sent from inside the process rather than by launching `osascript` — no Web API, no credentials. It mostly waits to be told: Spotify announces track and state changes, and between them the playback position is worked out from the clock, so it asks about once a second instead of three times a second. Lyrics come from [LRCLIB](https://lrclib.net), cached locally by track ID; syncs you tap out yourself live separately in `.user_syncs/` and are never treated as cache. Both sit in `~/Library/Application Support/SottoVoce/`, whichever way the app was started. The monitor and the lyrics provider know nothing about the UI. Display state, timing, menu gating, the type scale, geometry and the colour palettes live in pure, Qt-free modules behind a thin PySide6 window — which is why the contrast floor is a test rather than a judgement, and why the whole suite runs headless on Linux CI without touching the network, your settings or your Spotify ([how many, and how](docs/testing-and-ci.md)).
...
Also in `docs/`: [Spotify integration](docs/spotify-integration.md), [lyrics and caching](docs/lyrics-and-caching.md), [tap-to-sync](docs/tap-to-sync.md), [publishing](docs/publishing.md), [appearance and materials](docs/appearance-and-materials.md), [motion and typography](docs/motion-and-typography.md), [the compact layout and docking](docs/compact-and-docking.md), [per-app window position](docs/per-app-position.md), [yielding to notifications](docs/notification-yield.md), [getting out of the pointer's way](docs/pointer-yield.md), [the hotkey and Carbon](docs/hotkey-and-carbon.md), [the menu and system integration](docs/menu-and-system-integration.md), [the learning layers](docs/learning-features.md), [packaging](docs/packaging.md), [releasing](docs/releasing.md), and [Gatekeeper](docs/gatekeeper.md).
