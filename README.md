...
- **Album colour** — an optional layer that colours the window with the current cover's hue and cross-fades between tracks. Most of it lands on the hairline around the panel, which nothing is read against and which can therefore carry the hue properly; the glass itself takes a restrained wash. It uses the cover's *hue* only and decides for itself how much colour to carry, so a neon sleeve looks no louder than a muted one.
...
- **Album colour** colours the window from the current album cover and fades between tracks as they change. It is off by default like every other layer, and switching it off restores the plain window exactly. Only the *hue* comes from the cover — the panel keeps the brightness the window would have had anyway, which is why a black-and-white sleeve simply gets no tint (rather than a grey guess) and a fluorescent one gets a coloured window rather than a fluorescent one. Most of the colour is in the hairline around the panel: the panel's brightness is fixed by the promise about legibility and has almost nothing left to spend on colour, while the edge has no text on it and so carries the hue at four times the strength — the same amount in light mode as in dark. Covers are fetched once per track and only the resulting colour is stored, in `.artwork_cache/`; deleting it is a safe reset, like `.lyrics_cache/`.
