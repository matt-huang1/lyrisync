...
- **Album colour** — an optional layer that tints the panel with the current cover's hue and cross-fades between tracks. It takes the cover's *hue* only and keeps its own brightness, so a neon sleeve gives a coloured panel rather than a neon one, and the lyrics stay exactly as legible.
