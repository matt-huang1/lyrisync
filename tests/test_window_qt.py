...
def go_compact(window):
    ...
    """
    with patch.object(w.LyricsWindow, "_pointer_inside", lambda self: False):
        window._set_compact(True)


def hover(window, inside):
    """One turn of the pointer poll, with the pointer put where it is
    wanted.

    QCursor.pos() is whatever the offscreen platform last saw and cannot be
    driven from here, so the one thing that asks it is replaced.
    Everything above that is the real code, on the real timer's slot.
    """
    with patch.object(w.LyricsWindow, "_pointer_inside", lambda self: inside):
...
def test_the_pointer_is_only_watched_where_it_can_matter(make_window):
    ...
    window.apply_saved_visibility()
    land(window)
    assert window._hover_timer.isActive() is False  # compact is off
    ...
    land(window)
    assert window._hover_timer.isActive() is False
    window._set_lyrics_visible(True)
    land(window)
    assert window._hover_timer.isActive() is True
